		...
		<view class="content" v-show="show">
			<u-form :model="form" ref="uForm" label-width="140rpx" class="form form-top" label-position="top" v-if="form" :error-type="errorType">
				<view class="title bold u-border-bottom">
					<u-icon size="28" name="calendar" class="mar"></u-icon>
					<text>基本信息</text>
				</view>
				<u-form-item label="姓名"><u-input v-model="form.ygmingcheng" disabled /></u-form-item>
				<u-form-item label="工号"><u-input v-model="form.ygbianhao" disabled /></u-form-item>
				<u-form-item label="职位"><u-input v-model="form.gangwei" disabled /></u-form-item>
				<u-form-item label="所属店组"><u-input v-model="form.bumen" disabled /></u-form-item>
				...
				<u-form-item label="调店时间" :required="true" prop="sxshijian">
					<u-input v-model="form.sxshijian" placeholder="请选择时间" type="select" :select-open="isShowCalendar" @click="isShowCalendar = true" />
				...
				<u-form-item label="调入组名" :required="true" prop="groupName">
					<u-input :select-open="groupName" v-model="form.groupName" placeholder="请选择店组名" type="select" @click="actionSheet('groupName')" />
				...
				<u-form-item label="调动职务" :required="true" prop="zhiwuName">
					<u-input :select-open="zhiwu" v-model="form.zhiwuName" placeholder="请选择调动职务" type="select" @click="actionSheet('zhiwu')" />
					<u-select v-model="zhiwu" :list="form.zhiwu" @confirm="confirm2"></u-select>
				</u-form-item>
				<u-form-item label="调动类型" :required="true" prop="biandongName">
					<u-input :select-open="biandong" v-model="form.biandongName" placeholder="请选择店组名" type="select" @click="actionSheet('biandong')" />
				...
				<u-form-item label="调店原因" :required="true" prop="diao">
					<u-radio-group v-model="form.diao" @change="diaoChange">
						<u-radio v-for="(item, index) in leaveStatusList" :key="index" :name="item.value" shape="circle">{{ item.name }}</u-radio>
					</u-radio-group>
				</u-form-item>
				<u-form-item label="房源接收人" :required="true" prop="fzouName">
					<u-input :select-open="fzou" v-model="form.fzouName" placeholder="请选择房源接收人" type="select" @click="actionSheet('fzou')" />
					<u-select v-model="fzou" :list="form.yhlist" @confirm="confirm4"></u-select>
				</u-form-item>
				<u-form-item label="客源接收人" :required="true" prop="yzouName">
					<u-input :select-open="yzou" v-model="form.yzouName" placeholder="请选择客源接收人" type="select" @click="actionSheet('yzou')" />
					<u-select v-model="yzou" :list="form.yhlist" @confirm="confirm5"></u-select>
				</u-form-item>
				<u-form-item label="申请原因" :required="true" prop="bdyuanyin"><u-input v-model="form.bdyuanyin" type="textarea" /></u-form-item>
			</u-form>
				...
				bdyuanyin: [
					...
					},
					{
						...
						validator: (rule, value, callback) => {
							return this.$u.test.isEmpty(false);
						}
					}
				]
			},
